...
async function getStaticProps() {
  ...
  // Fetch API:
  const client = await external_mongodb_["MongoClient"].connect("mongodb+srv://quocbao19982009:Qscesz123@cluster0.g9eyr.mongodb.net/meetup?retryWrites=true&w=majority");
  const db = client.db();
  ...
  return {
    props: {
      meetups: meetups.map(meetup => ({
        ...
        address: meetup.address,
        image: meetup.image,
        id: meetup._id.toString() // ID return from meet ups need to be transfer to String

      }))
    },
    revalidate: 10000 // Use this if you need to reRender (ex: to fetch api again, for money price...)
...
/* harmony default export */ var pages = __webpack_exports__["default"] = (homePage); // domain.com/
